...
class LineGraphPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < labelY!.length; i++) {
      if (labelY![i].length.toDouble() > _offsetX) {
    ...
    drawXlab(canvas, margin, graph, cell); //jp added
    drawYlab(canvas, size, margin, graph, cell);
  ...
  void drawXlab(Canvas canvas, Size margin, Size graph, Size cell) {
    //draw xlab
    TextSpan span = new TextSpan(
      style: new TextStyle(
        color: graphColor,
    ...
    tp.paint(
        canvas,
        new Offset(
          margin.width + cell.width * labelX!.length / 2 - 16,
          margin.height + graph.height + 40,
        ));
  }

  void drawYlab(Canvas canvas, Size size, Size margin, Size graph, Size cell) {
    //draw xlab
    TextSpan span = new TextSpan(
      style: new TextStyle(
        color: graphColor,
